...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/13 19:04:23 by ukelly            #+#    #+#             */
/*   Updated: 2021/03/13 19:04:24 by ukelly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fxd			*fxd_new(size_t frac_size, int_fast16_t is_long_dbl)
{
	...
	ft_assert(fxd_new != NULL, __func__, "malloc error");
	...
	if (is_long_dbl)
	{
		...
		ft_assert(fxd_new->val != NULL, __func__, "malloc error");
...
void			get_floating_point_arg(t_ptf_info *info)
{
	...
	if (info->flags & SIZE_LDBL && IS_LONG_DBL_DEFINED)
	{
		...
		if (b80.s_pts.exp == 0x7FFF)
			fxd_ftoa_inf_nan(info, b80.s_pts.mant, b80.s_pts.sign);
		else if (fast && (info->prec + math_longlen(b80.val) <= LDBL_DIG))
	...
	else if (fast && (info->prec + math_longlen(b64.val) <= DBL_DIG))
