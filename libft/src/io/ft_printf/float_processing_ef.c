						...
						char *buff)
{
	...
	{
		j = math_intlen(fp->val[fp->f0 + i]);
		fp->val[fp->f0 + i] *= math_pow(10, FP_R_LEN - j);
		while (i < 0)
		{
			while (--j >= 0)
			{
						...
						const int_fast16_t inx, register int_fast32_t offset)
{
	t_fxd			*mul;
	int_fast16_t	i;
	const uint64_t	pw = math_pow(10, FP_R_LEN - (info->prec % FP_R_LEN));

	mul = fxd_new((inx < 0) ? -inx : 0, fp->f0 == FP_LD_POINT + 1);
	mul->frc_len = (inx < 0) ? -inx : 0;
	mul->int_len = (inx >= 0) ? inx + 1 : 0;
	mul->val[mul->f0 - 1 - inx] = math_pow(10, FP_R_LEN - offset);
						...
						char *buff)
{
	...
	if (fp->int_len == 0 && fp->frc_len > 0)
		while (fp->val[fp->f0 + inx] == 0 && inx < fp->frc_len)
			inx++;
	offset = math_intlen(fp->val[fp->f0 + inx]) - 1;
	...
	*buff++ = "-+"[inx < 0 || (fp->int_len == 0 && fp->frc_len == 0)];
	...
	if (fp->int_len == 0 && fp->frc_len == 0)
		...
		offset = math_abs(((inx > 0) ? inx : -(inx + 1)) * FP_R_LEN + offset);
	if (offset < 10)
		*buff++ = '0';
	inx = math_intlen(offset);
	while (inx-- > 0)
		*buff++ = (char)(math_divmod(offset, math_pow(10, inx), &offset) + '0');
	return (buff - ptr);
}
